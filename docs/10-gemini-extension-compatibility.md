...
O instalador deve ativar hooks Gemini conhecidos compatíveis de
`settings.json` e extensões por meio do plugin agentX. Scripts soltos continuam
...
| `settings.json` e `hooks/hooks.json` | hooks Gemini conhecidos via plugin agentX do OpenCode; eventos sem equivalente ficam no mapa |
...
- Hooks Gemini conhecidos de `settings.json` e extensões instaladas devem sincronizar automaticamente para OpenCode.
...
O modo padrao e sincronizar: hooks Gemini conhecidos de `settings.json`
e extensões rodam pelo plugin agentX do OpenCode sem etapa manual. Scripts soltos
