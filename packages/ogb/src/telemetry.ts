...
function diagnosticContext(workflow: string, payload: unknown, summary: Record<string, unknown>, status: string): Record<string, unknown> {
  ...
  if (includesAny(messagesText, ["opencode-auto-fallback is enabled", "plugin is not active", "plugin inactive"])) {
    ...
    recovery = "Rode agentx sync e reinicie o OpenCode se o aviso continuar.";
    ...
    recovery = "Hooks BeforeTool/AfterTool/BeforeAgent sincronizam automaticamente; revise scripts ou eventos sem equivalente OpenCode e registre hash legado so se quiser silenciar a auditoria.";
    ...
    recovery = "Nenhuma acao se rulesync foi desativado de proposito; rode agentx sync --rulesync auto para reativar.";
    ...
    recovery = "Rode agentx sync para regenerar arquivos com a versao atual.";
    ...
    recovery = "Rode agentx sync para regenerar os comandos do OpenCode.";
    ...
    recovery = "Rode agentx validate --json para ver quais checks avisaram.";
