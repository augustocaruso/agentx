...
function validateOgbGlobal(checks: ValidationCheck[], projectRoot: string, homeDir: string, options: { skipVersionCheck?: boolean } = {}): void {
  ...
  checks.push({
    ...
    message: result.error ?? (version === AGENTX_VERSION
      ...
      : `ogb resolves to ${resolved}, but reports ${version || "unknown version"}; expected ${AGENTX_VERSION}.`),
...
function validateHomeGlobalFiles(paths: ReturnType<typeof resolveProjectPaths>, checks: ValidationCheck[]): void {
  ...
  const expandedOk = Boolean(expandedText?.startsWith("# GENERATED BY OpenCode Gemini Bridge. DO NOT EDIT.") && expandedVersion === AGENTX_VERSION);
  ...
  checks.push({
    ...
    message: configReferencesInstruction(configPath, paths.expandedGeminiPath, paths.homeDir)
      ...
      : "Global OpenCode config does not reference the OGB expanded Gemini context. Run agentx sync.",
  });
...
export function runValidation(options: ValidationOptions = {}): ValidationReport {
  ...
  if (paths.homeMode) {
    ...
    checks.push({
      ...
      status: generatedConfig?._generated?.tool === "ogb" && generatedConfig?._generated?.version === AGENTX_VERSION ? "pass" : "fail",
