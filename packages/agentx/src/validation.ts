...
import { BINARY, DISPLAY, GENERATED_MARKDOWN_HEADER, GITHUB_REPO, LEGACY_BINARY, LEGACY_GENERATED_MARKDOWN_HEADER, RELEASE_ASSET } from "./brand.js";
...
import { STARTUP_SYNC_PLUGIN_FILENAME } from "./setup-opencode.js";
...
function configOgbStartupPluginState(configPath: string, homeDir: string): { ok: boolean; legacySpecs: string[] } {
  ...
  const expected = pathToFileURL(path.join(globalOpenCodeConfigDir({ homeDir }), "plugins", STARTUP_SYNC_PLUGIN_FILENAME)).href;
...
function validateOgbGlobal(checks: ValidationCheck[], projectRoot: string, homeDir: string, options: { skipVersionCheck?: boolean } = {}): void {
  const resolved = resolveCommand(BINARY, { homeDir, includeNpmPrefix: !options.skipVersionCheck });
  if (!resolved) {
    checks.push({ name: `${BINARY} global binary`, status: "warn", message: `${BINARY} is not available on PATH.` });
    return;
  }
  if (options.skipVersionCheck) {
    checks.push({
      name: `${BINARY} global binary`,
      status: "pass",
      message: `${BINARY} resolves to ${resolved}.`,
      details: { resolved, expectedVersion: AGENTX_VERSION },
    });
    return;
  }
  ...
  checks.push({
    name: `${BINARY} global binary`,
    status: result.error || result.status !== 0 || version !== AGENTX_VERSION ? "warn" : "pass",
    message: result.error ?? (version === AGENTX_VERSION
      ? `${BINARY} ${version} resolves to ${resolved}.`
      : `${BINARY} resolves to ${resolved}, but reports ${version || "unknown version"}; expected ${AGENTX_VERSION}.`),
    details: { resolved, expectedVersion: AGENTX_VERSION, reportedVersion: version },
  });
}
...
function validateHomeGlobalFiles(paths: ReturnType<typeof resolveProjectPaths>, checks: ValidationCheck[]): void {
  ...
  const expandedVersion = expandedText?.match(/^Generator:\s+(?:ogb|agentx)\s+(.+)$/m)?.[1]?.trim();
  const hasGeneratedHeader = expandedText?.startsWith(GENERATED_MARKDOWN_HEADER) || expandedText?.startsWith(LEGACY_GENERATED_MARKDOWN_HEADER);
  const expandedOk = Boolean(hasGeneratedHeader && expandedVersion === AGENTX_VERSION);
  ...
  checks.push({
    ...
    message: expandedText
      ...
      : "Missing global expanded Gemini context. Run agentx sync.",
  });
  ...
  checks.push({
    ...
    message: configReferencesInstruction(configPath, paths.expandedGeminiPath, paths.homeDir)
      ? `Global OpenCode config references the ${DISPLAY} expanded Gemini context.`
      : `Global OpenCode config does not reference the ${DISPLAY} expanded Gemini context. Run ${BINARY} sync.`,
  });
  ...
  checks.push({
    ...
    message: agentFiles.includes("YOLO") ? "Global YOLO agent file is present." : "Missing global YOLO agent file. Run agentx reset --yes.",
    details: { agentFiles },
  });
  checks.push({
  ...
  if (result.error || result.status !== 0) {
    ...
    if (blockedConfigDir && repairDirectoryBlocker(blockedConfigDir, paths, checks, "OpenCode config directory from debug error")) {
      ...
      checks.push({
        name: "OpenCode resolved config",
        status: "skip",
        message: `Skipped opencode debug config after the known Windows Bun mkdir EEXIST bug; ${DISPLAY} already validated the managed OpenCode files directly.`,
        details: {
...
function validateReleaseBootstrap(projectRoot: string, checks: ValidationCheck[]): void {
  ...
  const required = [
    ...
    ["bootstrap-mac.sh legacy cleanup", scripts.macBootstrap, "cleanup_legacy"],
    ["bootstrap-mac.sh keep legacy opt-in", scripts.macBootstrap, "--keep-legacy"],
    ...
    ["bootstrap-linux.sh legacy cleanup", scripts.linuxBootstrap, "cleanup_legacy"],
    ["bootstrap-linux.sh keep legacy opt-in", scripts.linuxBootstrap, "--keep-legacy"],
    ...
    ["install-posix.sh no extension update flag", scripts.posixInstaller, "--no-extension-update"],
    ...
    ["install-posix.sh stages stable CLI payload", scripts.posixInstaller, "copy_stable_cli_payload"],
    ["install-posix.sh serializes stable CLI swap", scripts.posixInstaller, "lock_dir=\"$parent_dir/.$base_name.install.lock\""],
    ["install-posix.sh copies stable CLI scripts", scripts.posixInstaller, "cp -R \"$source_dir/scripts\" \"$target_dir/scripts\""],
    ["install-posix.sh installs staged runtime dependencies", scripts.posixInstaller, "npm --prefix \"$target_dir\" install --omit=dev"],
    ["install-posix.sh swaps staged CLI install", scripts.posixInstaller, "mv \"$staging_dir\" \"$install_dir\""],
    ["install-posix.sh removes broken shim before wrapper", scripts.posixInstaller, "rm -f \"$PRIMARY_BIN\""],
    ["install-posix.sh removes legacy alias by default", scripts.posixInstaller, "rm -f \"$LEGACY_BIN\""],
    ["install-posix.sh keep legacy opt-in", scripts.posixInstaller, "WRITE_LEGACY_ALIAS"],
    ["install-posix.sh node shim fallback", scripts.posixInstaller, "exec node"],
    ["install-posix.sh version verification", scripts.posixInstaller, "Installed $BINARY_NAME at $PRIMARY_BIN, but it did not run"],
    ...
    ["uninstall-posix.sh removes CLI shims", scripts.posixUninstaller, "rm -f \"$PREFIX/bin/$BINARY_NAME\""],
...
function validateOptionalOpenCodeRun(projectRoot: string, homeDir: string, checks: ValidationCheck[]): void {
  ...
  const expected = "AGENTX_VALIDATE_OK";
  const result = run(opencode, ["run", "--agent", "YOLO", `Say exactly: ${expected}`], projectRoot, 120000);
  const output = `${result.stdout ?? ""}${result.stderr ?? ""}`;
  checks.push({
    name: "OpenCode live run",
    status: result.error || result.status !== 0 || !output.includes(expected) ? "warn" : "pass",
    message: result.error ?? (output.includes(expected) ? "OpenCode live run responded." : "OpenCode live run did not confirm the expected text."),
  });
}
...
export function runValidation(options: ValidationOptions = {}): ValidationReport {
  ...
  if (paths.homeMode) {
    ...
    checks.push({
      ...
      status: (generatedConfig?._generated?.tool === BINARY || generatedConfig?._generated?.tool === LEGACY_BINARY) && generatedConfig?._generated?.version === AGENTX_VERSION ? "pass" : "fail",
      message: generatedConfig?._generated?.tool === BINARY || generatedConfig?._generated?.tool === LEGACY_BINARY
        ? `Generated config version is ${generatedConfig._generated.version ?? "unknown"}.`
        : `Missing ${BINARY} generated config marker.`,
    });
  }
