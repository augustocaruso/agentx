...
function workflowDisplayName(workflow: string): string {
  ...
  return names[workflow] || workflow || `${DISPLAY} workflow`;
}
...
function workflowRecoveryCommand(workflow: string): string {
  const commands: Record<string, string> = {
    ...
    check: "agentx check",
    doctor: "agentx doctor",
    pass: "agentx check",
    "security-check": "agentx security-check",
    "setup-opencode": `${BINARY} setup-opencode`,
    sync: "agentx sync",
    validate: "agentx validate",
  };
  return commands[workflow] || `${BINARY} bridge`;
}
...
function diagnosticContext(workflow: string, payload: unknown, summary: Record<string, unknown>, status: string): Record<string, unknown> {
  ...
  if (includesAny(messagesText, ["opencode-auto-fallback is enabled", "plugin is not active", "plugin inactive"])) {
  ...
  } else if ((messagesText.includes("generated by ogb") && messagesText.includes("current ogb")) || (messagesText.includes("generated by agentx") && messagesText.includes("current agentx"))) {
  ...
  } else if (includesAny(messagesText, ["ogb global binary", "ogb resolves to", `${BINARY} global binary`, `${BINARY} resolves to`]) && messagesText.includes("expected")) {
    ...
    label = `${workflowDisplayName(workflow)} failed`;
    recovery = `Run ${workflowRecoveryCommand(workflow)} --json to see the diagnosis.`;
  } else if (status === "completed_with_warnings" || warnings.length > 0) {
    code = "workflow_warn";
    label = `${workflowDisplayName(workflow)} finished with warnings`;
    recovery = `Run ${workflowRecoveryCommand(workflow)} --json to see the next steps.`;
  }
  return {
    rootCauseCode: code,
...
export function printTelemetryStatus(status: TelemetryStatus, json = false): void {
  ...
  console.log(`${DISPLAY} telemetry: ${status.ready ? "ready" : status.enabled ? "enabled but not ready" : "disabled"}`);
...
export function printTelemetrySendResult(result: TelemetrySendResult, json = false): void {
  ...
  if (result.reason === "telemetry_not_enabled") {
    console.log(`${DISPLAY} telemetry: disabled or not ready.`);
    return;
  }
  console.log(`${DISPLAY} telemetry send: ${result.ok ? "ok" : "failed"}`);
