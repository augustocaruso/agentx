...
function addCheckOptions(command: Command): Command {
  return command
    ...
    .option("--no-antigravity-plugin-update", "Skip managed Antigravity plugin updates before sync")
...
async function runCheckCli(opts: CheckCliOptions, workflow: "check" | "pass", legacyWarning?: string): Promise<void> {
  ...
  await withWorkflowTelemetry(workflow, async () => {
    ...
    const run = (onProgress?: RitualProgressSink) => runPass({
      ...
      skipAntigravityPluginUpdate: opts.antigravityPluginUpdate === false,
...
telemetry.command("setup-email")
  ...
  .option("--worker-name <name>", "Cloudflare Worker name", "agentx-telemetry-email-worker")
...
program.command("setup-opencode")
  ...
  .option("--command <path>", `Command used by the startup plugin instead of the current ${BINARY} CLI`)
