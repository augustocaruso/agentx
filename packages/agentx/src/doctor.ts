...
import { AUTO_FALLBACK_PLUGIN, isManagedModelFallbackPlugin, resolveFallbackConfigPath } from "./external-integrations.js";
...
function hasConfiguredPlugin(plugins: string[], expected: string): boolean {
  if (isManagedModelFallbackPlugin(expected)) {
    return plugins.some((plugin) => isManagedModelFallbackPlugin(plugin));
  }
...
function readRuntimeFallback(projectRoot: string, homeDir: string) {
  ...
  const agentFallbacksSource = isManagedModelFallbackPlugin(pluginName) ? fallbackConfig?.agents : fallbackConfig?.agentFallbacks;
  const agentFallbacks = agentFallbacksSource && typeof agentFallbacksSource === "object" && !Array.isArray(agentFallbacksSource)
    ? Object.keys(agentFallbacksSource).length
    : 0;
  const defaultFallbacks = Array.isArray(fallbackConfig?.defaultFallback)
    ? fallbackConfig.defaultFallback.length
    : fallbackConfig?.agents && typeof fallbackConfig.agents === "object" && !Array.isArray(fallbackConfig.agents) && Array.isArray((fallbackConfig.agents as Record<string, any>)["*"]?.fallbackModels)
      ? (fallbackConfig.agents as Record<string, any>)["*"].fallbackModels.length
      : 0;
  return {
    configured,
    ...
    maxRetries: typeof fallbackConfig?.maxRetries === "number"
      ? fallbackConfig.maxRetries
      : typeof fallbackConfig?.defaults?.maxFallbackDepth === "number"
        ? fallbackConfig.defaults.maxFallbackDepth
        : undefined,
...
export function runDoctor(options: DoctorOptions = {}): DoctorReport {
  ...
  if (runtimeFallback.configured && !runtimeFallback.pluginActive) warnings.push(`model fallback is enabled in ${DISPLAY} config, but the OpenCode plugin is not active; run ${BINARY} setup-ux, then restart OpenCode.`);
  if (runtimeFallback.configured && !runtimeFallback.configExists) warnings.push(`model fallback config is missing: ${runtimeFallback.configPath}. Run ${BINARY} sync.`);
  if (runtimeFallback.configured && runtimeFallback.configEnabled === false) warnings.push("model fallback config exists but is disabled.");
