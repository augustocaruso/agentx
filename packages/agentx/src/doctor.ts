...
export function runDoctor(options: DoctorOptions = {}): DoctorReport {
  ...
  if (paths.homeMode) {
    if (hasGlobalGeminiContextSource(paths.homeDir, inv)) {
      ...
      else if (expandedGeminiVersion && expandedGeminiVersion !== AGENTX_VERSION) warnings.push(`Global expanded GEMINI file was generated by ogb ${expandedGeminiVersion}; current ogb is ${AGENTX_VERSION}. Run agentx sync.`);

      if (!fs.existsSync(opencodeConfig)) warnings.push("Missing global OpenCode config. Run ogb setup-ux or agentx sync.");
      else if (!configReferencesInstruction(opencodeConfig, paths.expandedGeminiPath, paths.homeDir)) warnings.push("Global OpenCode config does not reference the OGB expanded Gemini context. Run agentx sync.");
    }
    ...
    else if (expandedGeminiVersion && expandedGeminiVersion !== AGENTX_VERSION) warnings.push(`Expanded GEMINI file was generated by ogb ${expandedGeminiVersion}; current ogb is ${AGENTX_VERSION}. Run agentx sync.`);

    if (!fs.existsSync(paths.generatedOpenCodeConfigPath)) warnings.push("Missing .opencode/generated/opencode.generated.json. Run agentx sync.");
    else if (!generatedConfigHasMarker) warnings.push("Generated OpenCode config is missing ogb DO NOT EDIT metadata. Run agentx sync.");
    else if (generatedConfigVersion && generatedConfigVersion !== AGENTX_VERSION) warnings.push(`Generated OpenCode config was generated by ogb ${generatedConfigVersion}; current ogb is ${AGENTX_VERSION}. Run agentx sync.`);
  }
  ...
  else if (extensionMap?._generated?.version && extensionMap._generated.version !== AGENTX_VERSION) warnings.push(`Extension map was generated by ogb ${extensionMap._generated.version}; current ogb is ${AGENTX_VERSION}. Run agentx sync.`);

  if (state?.version && state.version !== AGENTX_VERSION) warnings.push(`Sync state was written by ogb ${state.version}; current ogb is ${AGENTX_VERSION}. Run agentx sync.`);
  ...
  if (!paths.homeMode && extensionCompatibility.modelFallbacks > 0 && !extensionCompatibility.modelRoutingReport) {
    warnings.push("Model fallbacks are configured, but OGB model routing report is missing. Run agentx sync.");
  } else if (modelRouting?.version && modelRouting.version !== AGENTX_VERSION) {
    warnings.push(`Model routing report was generated by ogb ${modelRouting.version}; current ogb is ${AGENTX_VERSION}. Run agentx sync.`);
  }
  ...
  if (runtimeFallback.configured && !runtimeFallback.configExists) warnings.push(`opencode-auto-fallback config is missing: ${runtimeFallback.configPath}. Run agentx sync.`);
  ...
  if (nativeCapabilityReport?._generated?.version && nativeCapabilityReport._generated.version !== AGENTX_VERSION) {
    warnings.push(`Native capability report was generated by ogb ${nativeCapabilityReport._generated.version}; current ogb is ${AGENTX_VERSION}. Run agentx sync.`);
  }
