...
test("planner contract builds a Windows install plan and exposes adapter details", () => {
  ...
  assert.equal(plan.adapter.generatedDir, "C:\\Users\\leona\\.config\\agentx\\generated");
...
test("planner contract builds update and check delegation commands", () => {
  ...
  assert.deepEqual(update.steps[1]?.command?.args, ["--project", projectRoot, "check", "--force"]);
